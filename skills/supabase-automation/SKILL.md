...
2. **Service role key vs anon key**
   ...
   - `service_role` key: Bypasses RLS (use for server-side ONLY)
...
9. **JSONB vs JSON**
   - Use `JSONB` (binary) for queryable JSON - faster indexing
   - Use `JSON` only for exact preservation of formatting
